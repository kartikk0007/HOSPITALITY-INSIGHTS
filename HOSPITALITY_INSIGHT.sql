...
# MONTH WISE OCCUPANCY RATE
SELECT MONTH(CHECK_IN_DATE) AS BOOKING_MONTH,
	   CONCAT(ROUND(((TOTAL_SUCCESSFUL_BOOKINGS/TOTAL_CAPACITY)*100),2),"%") AS OCCUPANCY_RATE
       FROM 
       (SELECT SUM(SUCCESSFUL_BOOKINGS) AS TOTAL_SUCCESSFUL_BOOKINGS,
        SUM(CAPACITY) AS TOTAL_CAPACITY FROM fact_aggregated_bookings) AS DERIVED_TABLE
        ,fact_bookings
        GROUP BY BOOKING_MONTH,OCCUPANCY_RATE;

/* NOT GETTING DESRIED OUTPUT */
...
SELECT WEEKDAY(BOOKING_DATE) AS WEEK_DAY, SUM(REVENUE_REALIZED) AS TOTAL_REVENUE
       FROM fact_bookings
       GROUP BY WEEK_DAY;
/* NOT GETTING THE RIGHT ANSWER HERE  */ 
